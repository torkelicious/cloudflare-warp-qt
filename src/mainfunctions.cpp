...
namespace {
    ...
    const QMap<QString, QString> kModeOutputNormalized = {
        ...
        {QStringLiteral("PostureOnly"), QStringLiteral("Device Information Only?")}
    };
}

MainFunctions::MainFunctions(QObject *parent) : QObject(parent) {
    resolvWatcher = new QFileSystemWatcher(this);
...
QString MainFunctions::runCommand(const QString &program,
    ...
    return runCommandResult(program, arguments).out;
}

MainFunctions::CommandResult
MainFunctions::runCommandResult(const QString &program,
                                const QStringList &arguments, const int timeoutMs) {
...
MainFunctions::runCommandAsync(const QString &program,
                               const QStringList &arguments, const int timeoutMs) {
    return QtConcurrent::run([program, arguments, timeoutMs] {
        return runCommandResultInternal(program, arguments, timeoutMs);
    });
}

void MainFunctions::setupToggleOperation(const QFuture<CommandResult> &future, bool isConnect) {
    QPointer safePtr(this);
    ...
    connect(
        ...
        [safePtr, watcher, isConnect] {
            const auto [exitCode, out, err, timedOut] = watcher->result();
            watcher->deleteLater();
            ...
            if (timedOut || exitCode != 0 ||
                !out.contains(QStringLiteral("Success"), Qt::CaseInsensitive)) {
...
QFuture<MainFunctions::CommandResult> MainFunctions::cliConnectAsync() {
    return runCommandAsync(QStringLiteral("warp-cli"), {QStringLiteral("connect")}, 15000);
}
...
QFuture<MainFunctions::CommandResult> MainFunctions::cliDisconnectAsync() {
    return runCommandAsync(QStringLiteral("warp-cli"), {QStringLiteral("disconnect")}, 15000);
}
...
QFuture<MainFunctions::CommandResult>
MainFunctions::cliStatusAsync(const int timeoutMs) {
    return runCommandAsync(QStringLiteral("warp-cli"), {QStringLiteral("status")},
                           timeoutMs);
}

void MainFunctions::checkResolvConf() {
    if (QFile file(QStringLiteral("/etc/resolv.conf")); file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        const QString content = file.readAll();
        resolvConnected = content.contains(QStringLiteral("nameserver 127.0.2.2")) ||
                          content.contains(QStringLiteral("nameserver 127.0.2.3"));
        file.close();
    }
}
...
bool MainFunctions::isServiceActive() {
    ...
    msg << QStringLiteral("warp-svc.service");

    const QDBusReply<QDBusObjectPath> reply = QDBusConnection::systemBus().call(msg);
    if (!reply.isValid()) {
        emit errorOccurred(QStringLiteral("Service Error"),
                           QStringLiteral("The 'warp-svc' service is not installed or "
                               "could not be found.\n\n"
                               "Please ensure Cloudflare WARP is installed correctly."));
        return false;
    }

    QDBusMessage propMsg = QDBusMessage::createMethodCall(
        QStringLiteral("org.freedesktop.systemd1"), reply.value().path(),
        QStringLiteral("org.freedesktop.DBus.Properties"), QStringLiteral("Get"));
    propMsg << QStringLiteral("org.freedesktop.systemd1.Unit")
            << QStringLiteral("ActiveState");

    if (const QDBusReply<QDBusVariant> propReply = QDBusConnection::systemBus().call(propMsg); propReply.isValid()) {
        if (const QString state = propReply.value().variant().toString(); state == QStringLiteral("active")) {
            return true;
        }
    }
...
QString MainFunctions::GetCurrentMode() {
    const CommandResult output = runCommandResult(QStringLiteral("warp-cli"),
                                                  {QStringLiteral("settings")});

    static const QRegularExpression re(
        QStringLiteral(R"(Mode:\s*([A-Za-z0-9]+))"));

    if (const QRegularExpressionMatch match = re.match(output.out); match.hasMatch()) {
        const QString outmatch = match.captured(1);
...
bool MainFunctions::isWarpConnected() const {
